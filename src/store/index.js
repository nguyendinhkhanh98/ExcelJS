...
const store = new Vuex.Store({
  ...
  mutations: {
    RUN_EXCELJS_EXPORT(state) {
      const { generate } =  useExcelExport();
      let data =[];
      // const data = Object.values(state.employees)
      for (var i=0; i < state.employees.length; i++) {
        const dataEmployee = Object.values(state.employees[i]);
        data.push(dataEmployee);
      }
      console.log(data)
      generate(data, {name: "test"}, {ref: "A1"});
    }
  },

  actions: {
    RunExcelJSExport({commit}) {
      commit("RUN_EXCELJS_EXPORT");
    }
  }
})
